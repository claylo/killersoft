...
class Amazon_FPS_Client implements Amazon_FPS_Interface
{
    ...
    private function _convertFundPrepaid($request) {
        ...
        if ($request->isSetFundingAmount()) {
            ...
            if ($fundingAmount->isSetCurrencyCode()) {
                $parameters['FundingAmount' . '.' . 'CurrencyCode'] =  $fundingAmount->getCurrencyCode();
            }
            if ($fundingAmount->isSetValue()) {
                $parameters['FundingAmount' . '.' . 'Value'] =  $fundingAmount->getValue();
            }
        }
        if ($request->isSetCallerReference()) {
            $parameters['CallerReference'] =  $request->getCallerReference();
        ...
        if ($request->isSetDescriptorPolicy()) {
            $descriptorPolicy = $request->getDescriptorPolicy();
            if ($descriptorPolicy->isSetSoftDescriptorType()) {
                $parameters['DescriptorPolicy' . '.' . 'SoftDescriptorType'] =  $descriptorPolicy->getSoftDescriptorType();
            }
            if ($descriptorPolicy->isSetCSOwner()) {
                $parameters['DescriptorPolicy' . '.' . 'CSOwner'] =  $descriptorPolicy->getCSOwner();
            }
        }
    ...
    private function _convertGetAccountActivity($request) {
        ...
        foreach  ($request->getRole() as $roleIndex => $role) {
            $parameters['Role' . '.'  . ($roleIndex + 1)] =  $role;
        }
    ...
    private function _convertPay($request) {
        ...
        if ($request->isSetTransactionAmount()) {
            ...
            if ($transactionAmount->isSetCurrencyCode()) {
                $parameters['TransactionAmount' . '.' . 'CurrencyCode'] =  $transactionAmount->getCurrencyCode();
            }
            if ($transactionAmount->isSetValue()) {
                $parameters['TransactionAmount' . '.' . 'Value'] =  $transactionAmount->getValue();
            }
        }
        ...
        if ($request->isSetMarketplaceFixedFee()) {
            $marketplaceFixedFee = $request->getMarketplaceFixedFee();
            if ($marketplaceFixedFee->isSetCurrencyCode()) {
                $parameters['MarketplaceFixedFee' . '.' . 'CurrencyCode'] =  $marketplaceFixedFee->getCurrencyCode();
            }
            if ($marketplaceFixedFee->isSetValue()) {
                $parameters['MarketplaceFixedFee' . '.' . 'Value'] =  $marketplaceFixedFee->getValue();
            }
        }
    ...
    private function _convertSettleDebt($request) {
        ...
        if ($request->isSetSettlementAmount()) {
            ...
            if ($settlementAmount->isSetCurrencyCode()) {
                $parameters['SettlementAmount' . '.' . 'CurrencyCode'] =  $settlementAmount->getCurrencyCode();
            }
            if ($settlementAmount->isSetValue()) {
                $parameters['SettlementAmount' . '.' . 'Value'] =  $settlementAmount->getValue();
            }
        }
        if ($request->isSetCallerReference()) {
            $parameters['CallerReference'] =  $request->getCallerReference();
    ...
    private function _convertWriteOffDebt($request) {
        ...
        if ($request->isSetAdjustmentAmount()) {
            ...
            if ($adjustmentAmount->isSetCurrencyCode()) {
                $parameters['AdjustmentAmount' . '.' . 'CurrencyCode'] =  $adjustmentAmount->getCurrencyCode();
            }
            if ($adjustmentAmount->isSetValue()) {
                $parameters['AdjustmentAmount' . '.' . 'Value'] =  $adjustmentAmount->getValue();
            }
        }
        if ($request->isSetCallerReference()) {
            $parameters['CallerReference'] =  $request->getCallerReference();
